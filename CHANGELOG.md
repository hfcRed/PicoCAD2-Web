...
- **Shader programs link in the background in every browser** — Browsers without `KHR_parallel_shader_compile` (Firefox, software rasterizers) used to block the page for the whole compile of every program. The library now queues a fence behind the link and polls it, so the link status is only read once the link has run in the command stream, and since capturing a frame would wait for the queued link too, the canvas keeps its last frame until then instead of the page blocking. Only the oldest queued link is polled, with the flush bit and on one timer shared by every viewer waiting on the context, which keeps Firefox from warning about unflushed sync polls. `shaderCompile: "sync"` still blocks. The wireframe's deform and glitch variants are now requested with the other programs, so `whenReady()` waits for them as well.
- **No compile at the first draw** — On Direct3D, ANGLE links a program for a single output and compiles it a second time, blocking, when its first draw goes into two attachments, which is what froze every tab for the length of a heavy material combination's compile after the effect had already appeared.
...
- **Leaving the canvas no longer stops a camera restore** — With `useFixedOnInteract`, a pointer leaving the canvas during the restore was handled as a release which cancelled the interpolation.
...
- **`source` is the parsed file** — `PicoCAD2ViewerState.source` is typed as the raw file object (`RawPicoCAD2File`) that `getState()` always returned, instead of a string. The object is the viewer's own parse of the file, made once at load and frozen, so `getState()` no longer parses the source on every call. It is typed read-only (`DeepReadonly<RawPicoCAD2File>`) to match. Copy it before editing it.
...
- **`maxFps` viewer option** — Caps how often `startRenderLoop()` draws (default: 60). Previously the loop drew on every animation frame, so high refresh rate displays caused periodic GC stutter in Firefox. Set to 0 to draw at the display refresh rate as before. Also available as `viewer.maxFps` and included in the viewer state (`viewer.maxFps`).
...
- **Display (`extras.display`)** — The video effects' screen simulation as a material effect, with `maskedColors` and `nodes`. Six screen types behind one `screenType` switch, the shared tone and grid controls, and the per-type settings that work on a surface. In `"uv"` `space` the surface is the screen, the virtual pixel grid lies on the texture, scanlines run along texel rows, the subpixel structure sits inside each cell, and a `resolution` coarser than the texture shows a coarser image. In `"screen"` space the structure follows the output pixels like the post effect. Warps, the projector's halo and ghosting stay with the post effect. Both effects run the same shaders.
- **Geometry effects** — Three new effects that reshape the model's geometry:
  - **Mesh deform (`extras.meshDeform`)** — Stackable geometry deforms. `voxel` remeshes the model into strict grid-aligned cubes (`gridSize` sets the voxel edge length, every cube keeps the color of the surface texel it replaced, transparent texels and hidden nodes produce no voxels, and all nodes share one grid so overlapping surfaces resolve to a single cube instead of z-fighting), plus the closed-form `barrel` bulge, `spherify`, and `twist` warps, which apply on top so a voxelized model can still bend. World space after the node transform, so hierarchy and animation stay correct. `progress` (default 1) runs the deform from untouched to full, by hand or through `cycle`, and the `sweep` group (see Sweeps) decides where the front is. The warps scale by the local progress at each vertex, and while the progress is partial every selected node is drawn from both its base mesh and its voxel stand-in, each keeping its side of the front per voxel cell, so whole cubes appear as a model voxelizes from one end, or a twist can travel up it as a wave.
  - **Triangle flash (`extras.triangleFlash`)** — Random triangles blink a color for a moment. Supports the material `style` setting allowing flashing triangles to keep their base palette index.
...
- **Directional outlines and drop shadows** — `extras.gradientOutline` gains `growthDirection` (radians, like `gradientDirection`) and `growthFactor` (0 = uniform, the previous behavior and default; 1 = one-sided) to grow the outline toward one side, plus a `mode` of `"outline"` or `"dropShadow"` with a `shadowOffset` in pixels — the drop shadow repeats the whole silhouette displaced by the offset for a sticker look.
- **Color scheme and dark outline colors** — `viewer.colorScheme` (`"light"`, `"dark"` or `"auto"`, default `"auto"`, also a constructor option and `viewer.colorScheme` in the state) tells the viewer which page scheme it renders for, so a model over a transparent background can match a page with light and dark modes. `"auto"` reads the browser's `prefers-color-scheme` on every frame. `extras.gradientOutline` gains a `dark` group (`enabled`, `colorFrom`, `colorTo`) that renders in place of `colorFrom` and `colorTo` while `dark.enabled` is set and the scheme resolves to dark. The built-in outline keeps the file's color, so nothing native to PicoCAD changes with the scheme.
- **Outlines fade with their content** — The built-in outline and `extras.gradientOutline` (drop shadow included) take the fade coverage of the pixels they trace instead of treating every drawn pixel as solid. Around a dissolving surface the outline continues the dither or the alpha of the fade instead of boxing every surviving pixel, runs ahead of the fade so it is gone by half coverage and never sits where the fade begins, and it fades around twinkling particles.
...
- **`transparency` viewer option** — Decides how every fade resolves against the background. `"dithered"` (default) claims whole pixels through an ordered dither, so the frame stays palette-pure and a GIF without alpha still shows the fade over a transparent background. `"smooth"` blends fractional alpha instead. A smooth dissolve draws the model in an opaque pass and a blended pass, so a fading surface shows what is behind it, and its floor shadow thins out through the checkerboard since a depth pass cannot blend. Available as a constructor option and `viewer.transparency`, included in the viewer state (`viewer.transparency`), exported as `TransparencyMode`.
...
- **Palette index buffer for custom effects** — The scene pass now writes a screen-space palette index buffer (R = base palette index, 255 = no model pixel, G = shade row, B = the pixel's fade coverage, 0 where nothing was drawn) available to post-process effects as `EffectContext.indexTexture`. `FullscreenEffect` passes it to shaders as `u_indexTexture` together with a `u_colorMask` bitmask packed from the effect's `maskedColors` (helper exported as `packColorMask`). A `SceneEffect` whose shader writes the index attachment as a second output declares `writesIndex`.
- **Public types completed** — `OrbitCamera` and `AnimationController` are exported so `viewer.camera` and `viewer.animation` can be typed by name.
...
- **Background shader compilation** — Shader programs link in the background where the browser supports parallel shader compilation, so enabling an effect or loading a model never freezes the page. Until a program is ready, frames draw with the programs they already have and the effect appears a few frames later. `new PicoCAD2Context({ shaderCompile: "sync" })` blocks on every compile instead. `viewer.whenReady()` resolves once the programs the current settings need are ready (await it before an export), `context.shadersReady` and `context.whenShadersReady()` report the context as a whole. Custom effects join in through `compilerFor(gl)`, which returns the context's `ProgramCompiler`, and a `ready` property the pipeline honors (`ManagedProgram` holds a program while it links). On a cold GPU process with the previous uber-shader, creating a context blocked for about 3 seconds and the first frame with an outline or a post effect another 3, measured on an RTX 5080 through Direct3D, the same loads now block for well under 100 ms. Creating a context links only the plain model program before returning, the outline program and the scene-target variant of the plain model follow in the background, so a first frame that already needs them may show the background alone.
...
- **Model shader variants** — The model, fur and wireframe programs are compiled per combination of enabled material and geometry effects, with each effect's code behind a preprocessor switch, instead of one program carrying every effect. A plain model compiles in milliseconds and a combination only carries what it uses, which is also what keeps the plain model as cheap per pixel as before the effects existed on software rasterizers. `MODEL_FEATURE` names the feature bits and `EffectContext.modelFeatures` carries the frame's, for scene effects that follow the deform or the glitch. Every model-pass feature (material, geometry, fur and floor effects) now renders through the scene target, so the direct-to-canvas path only ever draws the plain model and each featured variant has a single set of outputs. A smooth transparency fade of a dissolve or the floor without any post effect is now composited by the resolve pass instead of the canvas, which moves those pixels by at most two steps of 255. The floor's shadow map uses a depth-only variant that stops after the discards.
...
- **Less main-thread work per frame** — Render settings, shader uniform objects, and effect contexts are reused across frames instead of rebuilt every draw. Model buffers are drawn through vertex array objects with attribute locations bound before the programs link, so a draw binds one object instead of setting every attribute again. Animation clips are sorted per property and axis at parse time and only nodes that carry clips are evaluated and restored, so the other nodes keep their cached matrices. The camera distance clamp brings the ray into each mesh's space instead of every vertex into world space and skips meshes whose bounds the ray misses. Node selection bits and the pipeline's effect queries allocate nothing per frame. Tags are rendered once per text and color and copied per frame instead of filled pixel by pixel, the node uniforms and the face culling that repeat from draw to draw are only set when they change, clip easing is resolved at parse time, and post effects get the framebuffer they draw into as `EffectContext.framebuffer` instead of querying it. Measured on the example models at 128 pixels, a frame now takes about half the main-thread time of the 1.3.0 release and allocates a third of what the first 2.0.0 drafts did.
...
- **One path from a file to the viewer's settings** — `load()` applies the file's settings through the same code a state's `model` group goes through, so the two can no longer drift apart. Loading a file now resets `camera.zoom` to its default, which `modelInfo.settings` always claimed a file does.
- **`setState()` reuses the state's source** — A source written by `getState()` is loaded as the frozen object it is, without being serialized and parsed again, and a caller's own object is copied first so it stays editable. `onLoad` now fires once the state's settings are applied, instead of between the load and the restore.
...
- **A file that fails to parse leaves the viewer as it was** — `load()` used to free the previous model's resources before parsing, so an invalid file left the viewer reporting a model it could no longer draw.
- **Post effects survive zero settings** — A halftone `dotSize`, glitch `blockSize`, lens `zoom` or posterization `gamma` of 0, a depth fog `far` at or below `near`, and a chromatic aberration `radialFalloff` of 0 used to divide by zero in the shaders and blank or blacken the frame. They are clamped like the scene effects' settings.
- **Right-aligned tags with unknown characters** — A watermark containing a character the bitmap font lacks draws it as a question mark, but its width was measured without it, so a right-aligned tag ran past the canvas edge. Measuring and drawing now agree.
- **Viewer settings that used to break the frame** — A `cameraModeSpeed` of 0 (from a file, the property or a state) made the camera mode divide by zero and blank every frame. The mode now holds still instead. `setResolution()` clamps to whole pixels of at least one and falls back from a non-finite size or scale, instead of producing an empty canvas that made `toPixelData()` throw. Disabling the camera controls clears the pointer and inertia state, so the first gesture after re-enabling them no longer starts as a pinch against a stale point.
- **Panning under a camera mode** — Panning while a spin, sway or pingpong is running moved the target along the azimuth without the mode's offset, so the drag did not follow the screen axes. It now pans in the view the camera is actually showing.
...
- **Direct camera writes** — Setting `viewer.camera.omega`, `theta`, `distanceToTarget` or the components of `target` now moves the view. The cached view matrix used to ignore them until a control or a camera mode moved the camera. `theta` is kept short of the poles the way `rotate()` always did.
- **Model geometry freed on load and dispose** — Loading another model or disposing the viewer now deletes the previous model's vertex buffers and vertex array objects. They used to stay allocated on the GPU until the context was lost.
