...
- **Particles (`extras.particles`)** — Snow, rain, embers, sparkles or dust motes around the model. A single instanced draw of hashed, stateless, looping particles. Shapes `"pixel"` (constant screen size), `"quad"`, `"cube"` and `"triangle"`. Motions `"drift"`, `"rise"`, `"fall"`, `"orbit"` and `"swirl"`; `paletteIndices` colors particles from the model's palette, with `randomHue`/`hueRange` shifting each particle's hue by a random amount. `twinkle` fades particles in and out through alpha, and particles scale in when spawning and back out before despawning instead of popping.
