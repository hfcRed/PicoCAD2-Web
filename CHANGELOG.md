...
- **Interior effect (`extras.interior`)** — Fake depth behind selected palette colors. For masked texels the view ray is marched into the surface and a procedural 3D field (`"stars"`, `"dust"`, `"voronoi"`, `"lava"` or `"grid"`) is sampled at each depth, with parallax that tracks the camera.
